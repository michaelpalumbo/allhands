...
//TODO use a yarg called 'list' to get list of rooms currently in use, then process.exit()
...
// const local2Server = 7404
// const server2Local = 7403
...
// localSend = new Client('127.0.0.1', server2Local);
// console.log('Configure your local pd patch(es) to listen on UDP Port ' + server2Local)
...
ws.addEventListener('error', () => {
    console.log(`server went to sleep, waking please wait`);
});
...
ws.addEventListener('message', (data) => {
    ...
    switch (msg.cmd){
        ...
        case 'OSC':
            ...
            if(APRoot != name){
                // Max.outlet('incoming: ', msg.addressPattern, msg.typeTagString)

                // localSend.send(msg.addressPattern, msg.typeTagString, (err) => {
                //     if (err) console.error(err);
                // }); 
                Max.outlet('incomingData', msg.addressPattern, msg.typeTagString)
            }
...
// const localReceive = new Server(local2Server, '0.0.0.0');
// once running, inform user
// localReceive.on('listening', () => {    
//     console.log('Configure your local pd patch(es) to send on UDP Port ' + local2Server)
// })

// handle outgoing message:  

Max.addHandler('send', (OSC_AP, OSC_Data) => {
    ...
    if(OSC_AP.charAt(0) === '/'){
        ...
        Max.post('error, outgoing OSC Message must lead with an addressPattern\n\ni.e. /bioData')
    }
