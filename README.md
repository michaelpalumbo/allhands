...
<p>
  <img alt="Version" src="https://img.shields.io/badge/version-0.0.82-blue.svg?cacheSeconds=2592000" />
...
## Caveats:
If on OSX 10.15+, and using zsh as your terminal, you'll need to make sure that nodejs is in your PATH. See [this step-by-step before proceeding](https://medium.com/@andrewjaykeller/how-to-install-node-js-and-npm-with-macoss-new-terminal-zsh-e39b4a62d3d4)
...
```shell
npm install --location=global allhandsjs
```
...
1. **Server-Client (Legacy)** — connects to a shared allhands server and routes OSC data between all connected clients. Good for larger groups. 
2. **P2P** — sends OSC data directly between peers over WebRTC, with no data passing through the server after the initial connection. Good for low-latency one-to-one or mesh topologies for small group sessions.

3. **Coming soon: interoperability between the modes** - Members of your group can use either mode. 

---

## Configuration

*If using P2P Mode, skip ahead for those instructions, otherwise...*

### Server-Client Mode (Legacy)
...
P2P mode uses WebRTC to send OSC data directly between peers. After choosing P2P at startup, you'll be prompted for:

1. **Your name** — used to identify you on the network and prefix your OSC address patterns. No spaces.
2. **Send port** — the port your application (Max, Pd, Ableton, etc) uses to *send* OSC to allhands. Default is 7403.
3. **Receive port** — the port your application listens on for incoming OSC from peers. Default is 7404.

Once connected, allhands will wait for other peers to join. When a second peer connects, a direct WebRTC data channel is established between you and OSC data flows peer-to-peer from that point on.

In your application, send OSC to `127.0.0.1` on the send port (default 7403) and listen on the receive port (default 7404), just like in server-client mode.
